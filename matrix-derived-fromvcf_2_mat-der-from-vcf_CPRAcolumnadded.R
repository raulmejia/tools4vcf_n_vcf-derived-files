...
mymat <- read.table(input_mat_der_from_vcf, header=T, sep="\t") ### read
mymat_CPRA <- addCPRA_2_a_matrix(mymat) ## 

path_2_savefile <- paste0(result_path,"/",basename(input_mat_der_from_vcf),"_CPRAadded.tsv" )

write.table(file=path_2_savefile , mymat_CPRA, sep="\t",row.names = FALSE )
