...
only_vcfbasenames_paths <- your_vcfbasenames_paths[!grepl("tbi",your_vcfbasenames_paths)] # erasing the tbi files

names4cols <- lapply( only_vcfbasenames_paths , function(x){ str_remove(x,pattern_2_delete)}) # trimming the files' names
names4cols <- unlist(names4cols) # colnames for the final matrix

# reading the vcfs
vcfabsolute_paths <- paste0(inputvcfsfolder_path,only_vcfs_paths) # getting the absolute paths of the vcf files to be readed
vcf_list <- list() 
...
vcffix_onlyCP_list
length(vcffix_onlyCP_list)

as.character(vcffix_onlyCP_list[[k]])
as.character(vcffix_onlyCP_list[[23]])
...
heatmap(resultmatrix, scale="none")
